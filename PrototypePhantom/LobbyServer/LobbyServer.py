...
class Lobby:
    ...
    def request_keepalives(self, room_id):
        for i in self.rooms:
...
class Player:
    def __init__(self, player_name, character_id, ready, vote, address):
    ...
    def request_keepalive(self):
        ...
        if self.missed_keepalive_count > 2:
            print(f'Should remove player {self.player_name} from the server for idling, '
                  f'but the method to handle this hasn\'t been written...')
            # remove player
        else:
